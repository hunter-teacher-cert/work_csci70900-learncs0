...
public class BSTree {
  ...
  //a leaf point parent  to null
  //deleting a node with one child-point parent todeleted node child
  //find a node you can moveinto where deletion is
  //need a pointer above it
	if (root==null){
	    return;
	}
	...
	while (front != null && front.getData() != key ){ //stop if we find the key
	    if (front.getData() < key){//search to the right
		trailer = front;
		front = front.getRight();
  } else {//if greater move to the left
		trailer = front;
		front = front.getLeft();
	    }
	}
